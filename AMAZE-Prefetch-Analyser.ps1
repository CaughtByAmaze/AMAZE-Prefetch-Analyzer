...
try {
    ...
    Show-Header -Title "AMAZE Prefetch Analyzer"
    Write-Host "Version 1.0 | $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" -ForegroundColor DarkGray
    ...
    foreach ($File in $PrefetchFiles) {
        ...
        if ($LastModified -ne $LastRunTime) {
            $TimeMismatchFiles += "$($File.Name) (Last Run: $LastRunTime, Modified: $LastModified)"
            $SuspiciousCount++
        }
    }
