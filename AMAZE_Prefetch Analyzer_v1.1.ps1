...
# Set time tolerance for false positive reduction (45 seconds)
$TimeTolerance = New-TimeSpan -Seconds 45
...
try {
    ...
    Write-Host "Note: Using 45-second tolerance for time comparisons" -ForegroundColor DarkYellow
    Write-Host ""
    ...
    foreach ($File in $PrefetchFiles) {
        ...
        # Check time mismatches with tolerance
        $LastRunTime = $File.LastAccessTime
        $LastModified = $File.LastWriteTime
        $TimeDifference = $LastModified - $LastRunTime
        
        # Only flag if difference is greater than our tolerance window
        if ([math]::Abs($TimeDifference.TotalSeconds) -gt $TimeTolerance.TotalSeconds) {
...
Time Comparison Tolerance: $($TimeTolerance.TotalSeconds) seconds
